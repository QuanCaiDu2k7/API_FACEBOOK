...
mail = []
def fakeEmail(domain, total):
    faker = Faker()
    data_kytu = ['', '_', '.']
    for fake in range(total):
        number = random.randint(10,9999)
        first_name = faker.first_name().lower()
        last_name = faker.last_name().lower()
        kytu = random.choice(data_kytu)
        _mail_ = [
            f'{first_name}{kytu}{last_name}{number}@{domain}', 
            f'{first_name}{last_name}{kytu}{number}@{domain}', 
            f'{first_name}{kytu}{number}@{domain}', 
            f'{last_name}{kytu}{number}@{domain}', 
                ]
        mail_ = random.choice(_mail_)
        mail.append(mail_)
    return mail


def check_yahoo(mail_input):
    try:
        mail_output = mail_input[0:mail_input.index('@yahoo')]
        cookies = {
        ...
        check = requests.post('https://login.yahoo.com/account/module/create?validateField=yid', headers=headers,cookies=cookies, data=data)
        done = re.search('error',str(check.json()['errors']))
        if done == None:
            return json.dumps({'data':{'mail': mail_input, 'status': 'Die'}})
        else:
            return json.dumps({'data':{'mail': mail_input, 'status': 'Live'}})
    except:
        return json.dumps({'data':{'mail': mail_input, 'status': 'Wrong format!'}})
    
    
def check_hot(mail_input):
    try:
        mail_output = mail_input[0:mail_input.index('@hotmail')]
        check = requests.get(f"https://signup.live.com/signup?username={mail_output}@hotmail.com&lic=1")
        done = re.search("isAvailable",check.text)
        if done == None:
            return json.dumps({'data':{'mail': mail_input, 'status': 'Live'}})
        else:
            return json.dumps({'data':{'mail': mail_input, 'status': 'Die'}})
